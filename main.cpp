    ...
    for (unsigned i = 0; i < screenshots; i++) {
        for (unsigned test = 0; test < 10; test++) {
            ...
            if (success.has_value()) {
                write_image_png(IMGDIR + name + to_string(i) + ".png", *success);
 ...
 */
unordered_map<string, Tile<Color>> read_tiles(
    xml_node<>* root_node, const string& current_dir, const string& subset, unsigned size)
{
...
void read_simpletiled_instance(xml_node<>* node, const string& current_dir) noexcept
{
    ...
    ifstream config_file(RESOURCEDIR + name + "/data.xml");
    ...
    for (auto neighbor : neighbors) {
        ...
        neighbors_ids.push_back(
            make_tuple(tiles_id[neighbor1], orientation1, tiles_id[neighbor2], orientation2));
    }
    ...
    for (unsigned test = 0; test < 10; test++) {
        ...
        std::optional<Array2D<Color>> success = wfc.run();
        if (success.has_value()) {
            write_image_png(IMGDIR + name + "_" + subset + ".png", *success);
            cout << name << " finished!" << endl;
            break;
        } else {
            cout << "failed!" << endl;
...
void read_config_file(const string& config_path) noexcept
{
    ...
    std::cout << "Using this config file: " << config_path << std::endl;
...
// Initialize rand for non-linux targets
    ...
    read_config_file(RESOURCEDIR "samples.xml");
